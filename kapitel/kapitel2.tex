...
\begin{figure}[]
	\begin{center}
		\begin{tikzpicture}[scale=0.64,transform shape]
			...
			\path (p1.south)+(-2.5,-2.0) \blockitem{2}{{1}\phantomsection\label{step1}. Adapter trimming};
			\path (p2.south)+(2.5,-1.2) \blockitem{3}{{2}\phantomsection\label{step2}. Quality \\control};
			\path (p3.south)+(-2.5,-1.2) \blockitem{5}{{3}\phantomsection\label{step3}. Sequence alignment};
			\path (p3.west)+(4.75,-3.8) \blockitem{4}{{4}\phantomsection\label{step4}. Build graph};
			\path (p3.east)+(+9.8, 0.0) node (ur1)[ur] {\includegraphics[width=2.0cm]{bilder/qc_reports_icon_5.png}};
			\path (p4.south)+(0.0,-1.5) \blockitem{6}{{5}\phantomsection\label{step5}. Connected components};
			\path (p4.east)+(+7.0,0) node (ur2)[ur] {\includegraphics[width=2.0cm]{bilder/big_components_3.png}};
			\path (p6.south)+(-5.5,-1.4) \blockitem{7}{{7}\phantomsection\label{step7}. Allele \\combinations};
			\path (p6.south)+(0.0,-1.4) \blockitem{8}{{6}\phantomsection\label{step6}. Candidate \\alleles};
			\path (p6.east)+(+6.5,0) node (ur3)[ur] {\includegraphics[width=1.0cm]{bilder/small_components_2.png}};				
			\path (p7.south)+(0.0,-3.9) \blockitem{9}{{8}\phantomsection\label{step8}. Allele fractions};
			\path (p8.south)+(0.0,-1.5)	\blockitem{10}{{9}\phantomsection\label{step9}. Likelihood of alleles given one read};
			\path (p9.south)+(0.0,-7.64) \blockitem{11}{{14}\phantomsection\label{step14}. Indicator constrait};
			\path (p10.south)+(0.0,-1.5) \blockitem{12}{{10}\phantomsection\label{step10}. Likelihood of allele fractions given one read};
			\path (p11.south)+(0.0,-1.5) \blockitem{13}{{13}\phantomsection\label{step13}. Loci \\combinations};
			\path (p12.south)+(0.0,-1.5) \blockitem{14}{{11}\phantomsection\label{step11}. Likelihood of allele fractions given all reads};
			\path (p14.south)+(0.0,-1.5) \blockitem{15}{{12}\phantomsection\label{step12}. Maximum likelihood allele fractions};
			\path (p15.south)+(0.0,-2.0) \blockitem{16}{{15}\phantomsection\label{step15}. Likelihood \\of an allele given another allele};
			\path (p16.south)+(0.0,-1.8) \blockitem{17}{{16}\phantomsection\label{step16}. Likelihood of loci given alleles and fractions}; 
			\path (p17.south)+(0.0,-1.3) \blockitem{18}{{17}\phantomsection\label{step17}. Maximum likelihood loci};
			\path (p18.south)+(0.0,-1.7) \ioitem{19}{{18.}\phantomsection\label{step18} VCF \\ \includegraphics[width=0.9cm]{bilder/list_icon_2.png}\hspace{0.1cm}\includegraphics[width=0.9cm]{bilder/list_icon_2.png}\hspace{0.1cm}\includegraphics[width=0.9cm]{bilder/list_icon_2.png}};
...
Um die Wahrscheinlichkeit zu bestimmen, dass ein Read mit der Readsequenz $s_{r}$ und der Fehlerrate $\epsilon$ tatsächlich aus einem bestimmten Allel hervorgegangen (\hyperref[step9]{Schritt 9} des Workflows), muss die Sequenzierfehlerrate berücksichtigt werden. Die Zustandssequenz des approximierten pairHMM lässt sich dem CIGAR-String des Minimap2-Alignments entnehmen. Für jede Base der Querysequenz wird entsprechend den Operationen des CIGAR-Strings wird Sequenzierfehlerwahrscheinlichkeit auf unterschiedliche Weise in die Likelihoodberechnung einbezogen. \\
...
In Zusammenschau mit Formel \eqref{eqn:2-5} lässt sich beim paarweisen Vergleich eines Kandidatenallels $a_{i} \in A $ (siehe Kap. \ref{subsec:model_cand_allele}) mit einem Read mit der Sequenz $s_{r}$ die Likelihoodberechnung also wie folgt errechnen:
...
Ein weiteres Mal wird auf die Approximation des pairHMM durch Minimap2 zurückgegriffen, um die Menge der wahrscheinlichsten Allele einer Zusammenhangskomponente sinnvollen Locikombinationen zuordnen zu können (\hyperref[step15]{Schritt 15} des Workflows). Hierfür können bei diploiden Spezies die Allele ebenfalls paarweise unter Berücksichtigung der Hetereozygotiewahrscheinlichkeiten $\eta_{sub}$, $\eta_{ins}$ und $\eta_{del}$ mit einander verglichen werden. Auch hier wird die Sequenz eines der Allel (Query) gegen die Sequenz ein anderes Allels (Referenz) verglichen. Entsprechend den Operationen des CIGAR-Strings wird die Likelihood für jede Position $i$ der Querysequenz bestimmt. \\
...
Die Wahrscheinlichkeit $ pairHMM_{\eta}(a_{l_{j,query}} \, | \, a_{l_{j,ref}}) $ des  paarweisen Vergleichs zweier Allele hinsichtlich eines Locus $l_{j}$ errechnet sich wieder aus dem Produkt der Wahrscheinlichkeiten der einzelnen Basen:
...
Die Zusammenhangskomponenten können auch mehrere Loci enthalten, so dass in ihnen die einzelnen Allele mehrfach in verschiedenen Loci oder homozygot innerhalb eines Locus vorkommen können. Hinsichtlich der Allelkombinationen müssen also Wiederholungen der Allele erlaubt sein. Nach dem Urnenmodell werden dabei $n_{alleles}$ Allele mit Zurücklegen aus insgesamt $ n_{cand} $ verschiedenen Allelen kombiniert, wobei die Reihenfolge keine Relevanz hat. Dies wird in Kap. \ref{sec:max_lh_allele} detaillierter besprochen. \\

Für jede Allelkombination werden anschließend die relativen Häufigkeiten der darin enthaltenen Allele bestimmt (\hyperref[step8]{Schritt 8} des Workflows). Diese repräsentieren die Häufigkeitsverteilung der Allele der jeweiligen Kombination und werden im Folgenden als Allele-Fractions oder VAFs bezeichnet.\\

\subsubsection{Bestimmung der Likelihood eines Alleles im Bezug auf einen Read}

\noindent======================= draft =======================\\


Anschließend wird die Wahrscheinlichkeit bestimmt, dass ein Read tatsächlich von einem bestimmten Allel stammt. \\
